...
### v1.2.0 (2025-12-22)
- **核心稳定性升级**：重构下载与内存管理逻辑，极大降低内存占用。
- **大文件救援机制**：
  - 新增内存不足时的自动拦截功能，支持一键保存原始视频/音频轨道。
  - 优化文件名格式，自动添加 `[视频]` / `[音频]` 前缀，方便区分。
  - 智能交互流程：视频下载失败时可选择是否继续下载音频，避免重复劳动。
- **体验优化**：支持 File System Access API，大文件直接流式写入硬盘（需浏览器支持）。
