...
- **文件命名**：保存为 `视频-标题前5字.mp4` / `音频-标题前5字.mp3`（标题取视频标题，非网页标题）。
...
2.  **危险区/必爆区（> 20 分钟 4K / 长电影）**：
    ...
    *   **自动命名**：文件将被保存为 `视频-标题前5字.mp4` 和 `音频-标题前5字.mp3`，清晰易辨。
...
*   **无损合并（推荐）**：使用 FFmpeg 瞬间合并为标准 MP4（不消耗画质）：
    ```bash
    ffmpeg -i "视频-xxx.mp4" -i "音频-xxx.mp3" -c copy output.mp4
    ```
...
- **新增**：下载文件命名为 `视频-标题前5字.mp4` / `音频-标题前5字.mp3`（标题取视频标题）。
