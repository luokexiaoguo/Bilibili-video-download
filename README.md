...
- **合集批量下载**：一键下载整个合集或多P视频，自动列出所有分集供勾选，串行下载到同一个文件夹（支持 UGC 合集、视频集合、普通多P、番剧季、课程）。
...
### v1.2.15 (2026-09-05)
- **新增**：合集批量下载。弹窗新增「批量下载合集」按钮，可一键勾选整个合集/多P的全部分集，串行下载到同一文件夹。支持 UGC 合集、视频集合(series)、普通多P、番剧季、课程(cheese)。小文件自动合并为 MP4，大文件自动分轨保存，FFmpeg 单例复用，集间节流防风控。
- **修复**：分流下载时音频不再保存到浏览器默认下载文件夹，改为跟随视频保存到同一位置（弹两次保存窗口，与小贴士一致）。
- **修复**：分流下载中取消时不再卡住，overlay 会正常关闭。
- **优化**：修复反复下载时 overlay 与监听器叠加导致的内存泄漏（增加防重复注入守卫）。
- **优化**：下载进度更新增加节流，减少低配设备卡顿。

### v1.2.14 (2026-08-09)
- **修复**：修复不支持文件保存 API 的浏览器（Firefox / 老版 Chrome）启动分流下载时报错的问题（`fileHandle` 未声明即引用）。
- **修复**：修复无文件句柄兜底下载时保存 0 字节空文件、音视频实际未下载的问题，改为从流地址真正下载。
- **修复**：修复合并成功但写盘失败时，blob 兜底因 `const` 变量赋值报错、误弹"合并失败"分流提示的问题。

### v1.2.13 (2026-07-27)
- **修复**：修复合并成功后文件句柄写入失败（InvalidStateError）导致错误提示分流下载的问题。
- **修复**：修复番剧多集选择器不生效，始终下载第一集的问题。
- **修复**：修复 FFmpeg 因缺少 worker 文件导致合并失败的问题。
- **优化**：多集选择器支持番剧、UGC合集、UP主系列等更多视频类型。

### v1.2.12 (2026-07-25)
- **修复**：恢复 CORS 响应头规则（1002），修复部分 CDN 节点因缺少跨域头导致下载失败的问题。
- **优化**：为 DNR 规则（1001/1002）添加 initiatorDomains 限制，直播页面不再受扩展干扰。

### v1.2.11 (2026-07-25)
- **修复**：修复因 CORS 响应头重写导致部分视频 CDN 请求被拦截的 BUG。
- **优化**：精简 DNR 规则，仅保留必需的请求头修改。

### v1.2.10 (2026-07-09)
- **修复**：超大视频合并时内存溢出崩溃，改为流式直接写入磁盘。
- **修复**：直播页面无法播放（status 92002）的问题。
- **修复**：分流下载提示"没有权限"的 CDN 403 错误。
- **修复**：下载进度条卡在 90% 的问题。
- **修复**：下载进度显示闪烁的问题。
- **修复**：多次下载后页面卡顿的内存泄漏问题。
- **优化**：动态合并阈值，根据设备内存自动调整。
- **优化**：下载前快速探测文件大小，超出阈值立即提示分流。
- **优化**：CDN 请求多重兜底（credentials 回退 + 多域名重试），下载成功率更高。

### v1.2.9 (2026-06-29)
- **修复**：扩展导致 B 站直播页面无法播放的问题（status 92002）。修复了 declarativeNetRequest 规则错误拦截直播 CDN 请求和视频流媒体请求的 BUG。
